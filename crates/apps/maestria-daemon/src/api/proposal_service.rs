...
use maestria_domain::{
    DomainEvent, DomainInput, EvidenceId, KernelState, ModelAgentProposalExecution,
};
...
/// Submit an already-validated model-agent proposal to the runtime and assemble
/// the accepted response. Owns effect dispatch and response assembly; the
/// transport handler owns payload parsing and validation (R20).
pub(super) async fn submit_proposal(
    context: &ApiContext,
    proposal: ModelAgentProposal,
    task_validation: bool,
...
pub(super) async fn submit_approval(
    ...
    if identity.run_id != run_id {
        return Err(anyhow!(
            "approval {} belongs to model-agent run {}, not {run_id}",
            record.id,
            identity.run_id
        ));
    }
...
/// Read the terminal or in-flight status of a model-agent run.
