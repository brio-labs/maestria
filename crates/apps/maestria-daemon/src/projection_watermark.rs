...
pub(crate) fn finalize(layout: &InstanceLayout, state_snapshot: Option<serde_json::Value>) {
    ...
    if let Ok(store) = SqliteStore::open(&layout.database_path)
        && let Err(error) = store.set_projection_watermark(snapshot.to_string().as_str())
    {
        tracing::warn!(%error, "failed to advance projection reconciliation watermark");
    }
}
...
pub(crate) fn snapshot(
    ...
    let store = SqliteStore::open(&layout.database_path).ok()?;
