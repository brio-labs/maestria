...
pub(super) fn expand_graph(
    ...
    while let Some((endpoint, score, depth)) = queue.pop_front() {
        ...
        for relation in grounded_relations(ports, graph, endpoint)? {
...
fn grounded_relations(
    ...
    for relation in graph.get_relations_for(endpoint)? {
        ...
        if crate::retrieval::verify_source_snapshot(ports, &evidence).is_ok() {
...
fn project_graph_neighbor(
    ...
    match neighbor {
        ...
        RelationEndpoint::Artifact(artifact_id) => {
            ...
            if crate::retrieval::verify_source_snapshot(ports, &evidence).is_err() {
                return Ok(false);
            }
