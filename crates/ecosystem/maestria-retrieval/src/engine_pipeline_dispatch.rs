...
async fn collect_batches_serially(
    ...
    for (index, retriever) in retrievers.iter().enumerate() {
        ...
        if !lane_is_eligible(descriptor, plan, *web_requests_used) {
            ...
            batches.push(crate::types::CandidateBatch {
        ...
        let Some(allocation) = lane_budget(plan, *execution_usage, eligible_remaining.max(1), 0)
        else {
...
pub(crate) async fn collect_batches(
    ...
    if serial_dispatch_required(retrievers, plan, *execution_usage, *web_requests_used) {
        return collect_batches_serially(
            retrievers,
            plan,
            query,
            authorization,
