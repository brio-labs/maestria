...
fn record_complete_evidence_and_terminalize(
    ...
    for (order, excerpt, range) in [
        (0, "a", LineRange::new(1, 2)?),
        (1, "b", LineRange::new(2, 3)?),
    ] {
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    // A typed snapshot cannot be absent. Exercise the representable boundary
    // failure instead: evidence for the first chunk with a valid, but
    // semantically mismatched, full content hash must be rejected.
    let mismatched_ev_id = evidence_id_for(art_id, 0);
    let mismatched_hash = ContentHash::new("sha256:".to_owned() + &"f".repeat(64))?;
    let mismatch_result = state.apply_input(DomainInput::RecordEvidence(RecordEvidenceInput {
        evidence_id: mismatched_ev_id,
        artifact_id: art_id,
        claim_id: None,
        kind: EvidenceKind::FileSpan {
            path: "/tmp/notes.md".to_string(),
            range: LineRange::new(1, 2)?,
            snapshot: SnapshotRef::new(BlobId::new(42), mismatched_hash),
        },
    ...
    assert!(matches!(
        ...
        }) if evidence_id == mismatched_ev_id
    ));

    // Still blocked — chunk[0] evidence remains missing after rejection.
    ...
    assert_eq!(
        ...
        IndexStatus::Pending,
        "rejected mismatched evidence must not satisfy terminalization gate"
    );
    assert!(
        !output3
            .events
            .iter()
            .any(|e| matches!(e.event, DomainEvent::ArtifactIndexed { .. })),
        "no ArtifactIndexed after mismatched evidence is rejected"
    );
