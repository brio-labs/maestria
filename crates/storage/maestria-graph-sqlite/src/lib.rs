...
fn migrate(connection: &mut Connection) -> Result<(), PortError> {
    connection
        .execute_batch(
            "CREATE TABLE IF NOT EXISTS graph_projection_schema (
             ...
             INSERT INTO graph_projection_schema (id, version)
             VALUES (1, 1)
    ...
    let version = connection
        ...
        .ok_or_else(|| PortError::Internal {
            message: "graph projection schema version missing".to_string(),
        })?;

    if version != SCHEMA_VERSION {
        return Err(PortError::Internal {
            message: format!("unsupported graph projection schema version {version}"),
        });
    }
