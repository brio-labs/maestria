...
use crate::{
    error::to_port_error,
    schema::{
        FIELD_ARTIFACT_ID, FIELD_CARD_ARTIFACT_ID, FIELD_CARD_BODY, FIELD_CARD_FILENAME,
        FIELD_CARD_ID, FIELD_CARD_KEY, FIELD_CARD_PATH, FIELD_CARD_SYMBOL, FIELD_CARD_TITLE,
        FIELD_CHUNK_ID, FIELD_FILENAME, FIELD_PATH, FIELD_SYMBOL, FIELD_TEXT, schema_field,
    },
};
...
pub(super) fn legacy_chunks(index: &Index) -> Result<Vec<IndexedChunk>, PortError> {
    ...
    for (_, address) in documents {
        ...
        let artifact_id = document
            ...
            .ok_or_else(|| {
                PortError::internal(
                    "decode legacy chunk artifact id",
                    "legacy chunk is missing artifact id",
                )
            })?;
        let chunk_id = document
            ...
            .map(ChunkId::new)
            .ok_or_else(|| {
                PortError::internal("decode legacy chunk id", "legacy chunk is missing chunk id")
            })?;
        let text = document
            ...
            .map(str::to_string)
            .ok_or_else(|| {
                PortError::internal("decode legacy chunk text", "legacy chunk is missing text")
            })?;
