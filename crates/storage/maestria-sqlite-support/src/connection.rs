...
/// busy timeout (concurrent projections share the database) and WAL journal
/// mode. Schema migration stays with the owning crate.
...
/// Opens an in-memory SQLite database with the same busy-timeout and WAL
/// policy as [`open_connection`].
