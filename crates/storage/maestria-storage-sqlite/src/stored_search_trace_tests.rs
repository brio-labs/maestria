...
use maestria_domain::{
    ...
    DiversityPlacement, DuplicateClusterId, EvidenceId, EvidenceRequirements, EvidenceSpan,
    FreshnessRequirement, FreshnessStatus, IndexGenerationId, Modality, ModalitySet, QueryId,
    RepresentationName, RerankPosition, RetrievalLaneScore, RetrievalModelFingerprint,
    RetrievalRawRank, RetrievalReason, RetrievalScoreFingerprint, RetrievalScoreKind,
    RetrievalScoreScale, RetrievalScoreSet, SearchBudget, SearchBudgetLimits, SearchDegradation,
    SearchExecution, SearchExecutionBudget, SearchExecutionCompletion, SearchExecutionUsage,
    SearchIntent, SearchLaneStatus, SearchRewriteAccounting, SearchRewriteOrigin,
    SearchRewriteStage, SearchStage, SearchStopReason, SearchTrace, SearchTraceCandidate,
    SearchTraceCandidateDto, SearchTraceConstraintScore, SearchTraceDiversity,
    SearchTraceDiversityCandidate, SearchTraceExpansion, SearchTraceFilter, SearchTraceLane,
    SearchTraceLaneCandidate, SearchTraceLaneCandidateDto, SearchTraceRerank,
    SearchTraceRerankCandidate, SearchTraceRewrite, SourceLocation, StopConditions,
...
pub(crate) fn sample_trace() -> Result<SearchTrace, Box<dyn std::error::Error>> {
    ...
    Ok(SearchTrace {
        ...
        degradation: Some(SearchDegradation {
            capability: "visual".to_owned(),
            reason: "visual->text".to_owned(),
        }),
        stages: vec![
...
pub(crate) fn sample_trace_diversity() -> SearchTraceDiversity {
    SearchTraceDiversity {
        ...
        candidates: vec![SearchTraceDiversityCandidate {
            candidate_id: EvidenceId::new(1),
            original_rank: 0,
            placement: DiversityPlacement::Selected(0),
            duplicate_cluster: Some(DuplicateClusterId::new(5)),
            marginal_coverage: 3,
            coverage_keys: vec!["k-1".to_owned()],
        }],
