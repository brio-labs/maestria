...
impl SqliteVectorIndex {
    ...
    fn lock_connection(&self) -> Result<MutexGuard<'_, Connection>, PortError> {
        self.connection.lock().map_err(|_| PortError::Internal {
            message: "vector sqlite connection lock poisoned".to_string(),
        })
    }
}
...
impl VectorIndex for SqliteVectorIndex {
    fn index_embeddings(&self, embeddings: Vec<VectorEmbedding>) -> Result<(), PortError> {
        ...
        let transaction = connection.transaction().map_err(to_port_error)?;
        {
            let mut statement = transaction
                .prepare(
                    "INSERT INTO vector_embeddings (chunk_id, dimension, embedding)
                     VALUES (?1, ?2, ?3)
    ...
    fn search_similar(&self, query: VectorSearchQuery) -> Result<Vec<VectorSearchHit>, PortError> {
        ...
        hits.sort_by(|left, right| match right.score.partial_cmp(&left.score) {
...
fn migrate(connection: &mut Connection) -> Result<(), PortError> {
    connection
        .execute_batch(
            "CREATE TABLE IF NOT EXISTS vector_projection_schema (
             ...
             INSERT INTO vector_projection_schema (id, version)
             VALUES (1, 1)
    ...
    let version = connection
        ...
        .ok_or_else(|| PortError::Internal {
            message: "vector projection schema version missing".to_string(),
        })?;

    if version != SCHEMA_VERSION {
        return Err(PortError::Internal {
            message: format!("unsupported vector projection schema version {version}"),
        });
    }
...
fn sqlite_vec_available(connection: &Connection) -> Result<bool, PortError> {
    let exists = connection
        .query_row(
            "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'vec_docs'",
            [],
            |row| row.get::<_, i64>(0),
        )
        .optional()
...
mod tests {
    use maestria_domain::ChunkId;
    use maestria_ports::{PortError, VectorEmbedding, VectorIndex, VectorSearchQuery};

    use super::SqliteVectorIndex;

    #[test]
    fn indexes_and_searches_by_similarity() -> Result<(), PortError> {
        ...
        assert_eq!(hits[1].chunk_id, ChunkId::new(3));
        assert!(hits[0].score >= hits[1].score);
        Ok(())
    }

    #[test]
    fn replaces_existing_embedding_for_chunk() -> Result<(), PortError> {
        let index = SqliteVectorIndex::in_memory()?;
    ...
    #[test]
    fn rejects_invalid_vectors() -> Result<(), PortError> {
        let index = SqliteVectorIndex::in_memory()?;
    ...
    #[test]
    fn search_filters_dimension() -> Result<(), PortError> {
        let index = SqliteVectorIndex::in_memory()?;
        index.index_embeddings(vec![VectorEmbedding {
            chunk_id: ChunkId::new(1),
            vector: vec![1.0, 0.0, 0.0],
