...
Mechanical checks conservatively reject opposite boolean state pairs, known boolean/optional-payload state pairs, string-backed fields named as state discriminants, and multiple same-primitive `_id` values that can be swapped within one kernel struct or function. They intentionally do not infer correlation between arbitrary fields or constructor correctness; those remain mandatory review obligations because a broad syntactic ban would reject valid models.
...
- Type-invariant checker tests cover opposite boolean states, known boolean/optional-payload state pairs, stringly state discriminants, swappable primitive identities, and legitimate independent flags, optional values, and typed IDs; review covers arbitrary correlated payloads, constructor visibility, and whether typestate complexity is justified.
