#!/usr/bin/env python3
"""Compute the locked artifact hash required by a sparse model profile."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--profiles",
        default=Path(__file__).with_name("sparse_model_profiles.json"),
        type=Path,
    )
...
def main() -> int:
    ...
    if not isinstance(profile, dict):
        raise SystemExit(f"unknown sparse model profile: {args.profile}")
