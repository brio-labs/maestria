            ...
            return match err {
                ...
                ReplyError::X11Error(err) => {
                    if let ErrorKind::Access = err.error_kind {
                        ...
                        for m in ignored_mods() {
                            let failure = match conn.ungrab_key(keycode, root, mods | m) {
                                Ok(reply) => reply.check().err().map(|error| error.to_string()),
                        ...
                        if let Some(error) = rollback_failure {
                            Err(Error::FailedToRegister(format!(
                                "Hotkey collision cleanup failed: {error}"
                            )))
                        } else {
...
fn unregister_hotkey(
    ...
    for m in ignored_mods() {
        conn.ungrab_key(keycode, root, modifiers | m)
            .map_err(|error| {
                Error::OsError(std::io::Error::other(format!(
                    "Failed to ungrab hotkey {hotkey:?}: {error}"
                )))
            })?
            .check()
            .map_err(|error| {
                Error::OsError(std::io::Error::other(format!(
                    "Failed to confirm hotkey ungrab {hotkey:?}: {error}"
                )))
            })?;
    }
